      ...
      <div className="mx-auto flex w-full max-w-6xl items-center justify-between gap-4 px-4 py-3 sm:px-6">
        ...
        <nav aria-label="Principal" className="flex items-center gap-2 sm:gap-3">
          {!compact && (
            <Link
              href="/#como-funciona"
              className="hidden rounded-md px-3 py-2 text-sm text-mist transition hover:text-ink sm:inline-flex"
            >
              Cómo funciona
            </Link>
          )}
          <Link
            href="/analizar"
            className="inline-flex items-center rounded-md bg-lime px-3 py-2 text-sm font-semibold text-pitch-950 transition hover:bg-lime-dim"
          >
            Analizar mi equipo
          </Link>
        </nav>
...
export function SiteFooter() {
  return (
    <footer className="mt-auto border-t border-[var(--line)] bg-pitch-950/80">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-3 px-4 py-8 text-sm text-mist sm:px-6">
